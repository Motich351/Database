...
from PyQt5.QtWidgets import QWidget, QTableWidgetItem, QMessageBox, QDialog
from sqlalchemy import or_

from database import Worker, create_session, global_init, Shop
from ui_Worker import Ui_WorkerAdd
from ui_WorkerTab import Ui_Form


class WorkerTab(QWidget, Ui_Form):
    def __init__(self):
        super().__init__()
        self.setupUi(self)
        self.table_is_changeable = True
        self.session = None
        self.worker_adder = None
    ...
    def delete_worker(self, delete_by_search=True):
        ...
        if delete_by_search:
            ...
            workers = [
                self.session.query(Worker).get(int(self.tableWorker.item(row.row(), 0).text())) for
                row in
                selected_rows]
